...
function ensureAuthenticated(req, res, next) {
  if (req.isAuthenticated()) {
    ...
    return res.status(401).json({ message: 'Para agregar a favoritos debe iniciar sesión' });
  }
}
...
app.post('/addmovie', ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Película añadida correctamente a favoritos' });
  } catch(err) {
    console.error(err);
...
app.get('/addmovie/:title', ensureAuthenticated, async (req, res) => {
  ...
  if(result.length > 0) {
      ...
      if(favResult.length > 0) {
          res.json({ message: 'La película ya está en la lista de favoritos' });
      } else {
          query = `INSERT INTO favoritos (pelicula_id, usuario_id) VALUES (?, ?)`;
          await db.query(query, [movieId, userId]);
          res.json({ message: 'Película añadida correctamente a favoritos' });
